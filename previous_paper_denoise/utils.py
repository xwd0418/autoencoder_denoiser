...
def logging_setup(log_dir):
    current_time_str = strftime("%Y_%m_%d_%H_%M_%S", localtime()) #创建此时此刻的年、月、日、时、分、秒的字符串
    ...
    log_all = logging.getLogger('log_all') #初始化日志
    log_all.setLevel(logging.DEBUG) #设置日志等级为DEBUG
